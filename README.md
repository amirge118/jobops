...
company ATS boards, simple official career pages, and selected WhatsApp groups, verifies job pages, scores each role against
the candidate profile, removes repeats, and produces a short Hebrew report. A local web
dashboard exposes the same workflow across focused scan, decision, and company pages without
requiring terminal commands.

The dashboard also maintains a persistent company watchlist. A company discovered from a
WhatsApp job or entered manually is first shown as a candidate; only an explicit confirmation
adds its supported job source to future scans.
...
[`docs/company-expansion-research.md`](docs/company-expansion-research.md).
...
[`docs/adr/0002-multi-page-dashboard.md`](docs/adr/0002-multi-page-dashboard.md).
...
[Collector runbook](docs/whatsapp-collector-runbook.md) and
[architecture decision](docs/adr/0001-persistent-whatsapp-collector.md).
