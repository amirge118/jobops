 ...
 *   - fetch(entry, ctx): [{title,url,company,location,postedAt?}] — required
 *
 ...
 *   node scan.mjs --max-age=48     # only keep postings from the last 48h (needs postedAt
 *                                   from the provider; overrides portals.yml recency_filter)
 */
...
// ── Recency filter ──────────────────────────────────────────────────
// Optional. Off by default — undefined maxAgeHours passes everything, matching
// today's behavior. A job with no postedAt (provider doesn't expose a date, or
// a source that hasn't been wired up yet) also passes — same "don't penalize
// missing data" policy as the location filter above.
export function buildRecencyFilter(maxAgeHours) {
  if (!maxAgeHours && maxAgeHours !== 0) return () => true;
  const maxAgeMs = maxAgeHours * 60 * 60 * 1000;

  return (postedAt) => {
    if (typeof postedAt !== 'string' || postedAt.trim() === '') return true;
    const posted = Date.parse(postedAt);
    if (Number.isNaN(posted)) return true;
    return Date.now() - posted <= maxAgeMs;
  };
}
...
function appendToPipeline(offers) {
  ...
  // Keep the shared pipeline vocabulary consistent with the rest of jobOps.
  const marker = '## Pending';
  const idx = text.indexOf(marker);
  if (idx === -1) {
    // No Pending section — append at end before Done.
    const procIdx = text.indexOf('## Done');
...
function appendToScanHistory(offers, date, status = 'added') {
  // Ensure file + header exist. Location appended as 7th column, date_posted as 8th,
  // both for non-breaking backward compat — older scan-history.tsv files with fewer
  // columns still parse fine since loadSeenUrls only reads column 0. `status` is
  // parameterized so callers can record verify outcomes (`skipped_expired`, etc.)
  // without the legacy `(expired)` suffix in `source`.
  if (!existsSync(SCAN_HISTORY_PATH)) {
    writeFileSync(SCAN_HISTORY_PATH, 'url\tfirst_seen\tportal\ttitle\tcompany\tstatus\tlocation\tdate_posted\n', 'utf-8');
  }

  const lines = offers.map(o =>
    `${o.url}\t${date}\t${o.source}\t${o.title}\t${o.company}\t${status}\t${o.location || ''}\t${o.postedAt || ''}`
  ).join('\n') + '\n';
...
export async function runPortalScan(args = [], { additionalCompanies = [] } = {}) {
  ...
  // --max-age=<hours>: one-off recency override, e.g. `--max-age=48` for "posted in
  // the last 48 hours". Takes precedence over portals.yml's recency_filter so it can
  // be used ad hoc without editing config. Falls through to portals.yml (then unset)
  // when absent.
  const maxAgeArg = args.find((a) => a.startsWith('--max-age='));
  const cliMaxAgeHours = maxAgeArg ? Number(maxAgeArg.slice('--max-age='.length)) : undefined;

  // 1. Load providers
  const providers = await loadProviders(PROVIDERS_DIR);
  if (providers.size === 0) {
    throw new Error('No providers loaded from providers/');
  }
  ...
  const maxAgeHours = cliMaxAgeHours !== undefined && !Number.isNaN(cliMaxAgeHours)
    ? cliMaxAgeHours
    : config.recency_filter?.max_age_hours;
  const recencyFilter = buildRecencyFilter(maxAgeHours);
  ...
  // 4. Load dedup sets
  const seenUrls = ignoreHistory ? new Set() : loadSeenUrls();
  const seenCompanyRoles = ignoreHistory ? new Set() : loadSeenCompanyRoles();
  ...
  if (maxAgeHours !== undefined) {
    log(`Filtered by recency:   ${totalFilteredRecency} removed (older than ${maxAgeHours}h)`);
  }
  ...
  if (verifiedOffers.length > 0) {
    ...
    for (const o of verifiedOffers) {
      log(`  + ${o.company} | ${o.title} | ${o.location || 'N/A'} | posted ${o.postedAt || 'unknown'}`);
    }
    if (dryRun) {
  ...
  log(`\n→ Run npm run jobs to evaluate new offers.`);
  ...
  return {
    ...
    stats: {
      ...
      filteredRecency: totalFilteredRecency,
...
if (import.meta.url === pathToFileURL(process.argv[1] || '').href) {
  runPortalScan(process.argv.slice(2)).catch(err => {
